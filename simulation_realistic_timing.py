...
class SimulationEngine:
    ...
    def __init__(self, num_nodes, packet_prob, node_type='BEB', 
        ...
        # If user provides "per-slot" probability, we need to scale it
        # Original slot was abstract; now we generate per µs
        # To keep same average rate: prob_per_µs = prob_per_slot / slot_duration
        # For backward compatibility, assume user's prob is "per abstract time unit"
    ...
    def step(self):
        ...
        if ended:
            # Provide feedback to nodes
            if final_status == Channel.SUCCESS:
                self.successful_transmissions += 1
                # Success: node enters ACK wait, but we simplify by immediate feedback
        ...
        # 5. Record metrics snapshot (every 100ms = 100,000 µs)
        if (t + 1) % 100000 == 0:
    ...
    def _record_history(self, t):
        ...
        # Get metrics since last snapshot
    ...
    def _calculate_final_metrics(self):
        ...
        throughputs = [n.total_success / (self.duration / 1e6) for n in self.nodes]
        ...
        print(f"Fairness Index: {self.metrics['fairness']:.4f}")
